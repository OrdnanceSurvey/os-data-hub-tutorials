...
var sliderLeft = $(".osel-sliding-side-panel.panel-left").slideReveal({
  ...
  shown: function () {
    ...
    if (typeof sliderLeftShownHandler === "function") sliderLeftShownHandler();
  },
  hide: function (obj) {
    obj
      .find("div.toggle-handle")
...
var sliderRight = $(".osel-sliding-side-panel.panel-right").slideReveal({
  ...
  show: function (obj) {
    ...
    if (isLeaflet) map.closePopup();
    else if (isMapboxGL) $(".mapboxgl-popup").remove();
...
$(
  ...
  $("ul.layers .layer .layer-element[data-type='radio']").attr(
    "data-state",
    "unchecked"
  );
...
$(
  ...
  if (container.hasClass("layer")) {
    container
      .find('.layer-element[data-type="checkbox"]')
    ...
    siblings.each(function () {
      if ($(this).attr("data-state") == "checked") counter++;
    });

    var parent = container.parent();
    if (counter == 0) checked = "unchecked";
    else if (counter == siblings.length) checked = "checked";
    else checked = "indeterminate";
...
$(".osel-sliding-side-panel .osel-toggle-layers div").click(function () {
  var checked =
      $(this).attr("id") == "show-all-layers" ? "checked" : "unchecked",
    elem = $(
      ".osel-sliding-side-panel.panel-left .layer .layer-element[data-type='checkbox']"
    );
  ...
  elem.each(function () {
    if ($(this).parent().hasClass("layer")) toggleLayer($(this), "checkbox");
  });

  if ($(this).attr("id") == "clear-all-layers") return;
...
$(
  ...
  var elem = $(
    ".osel-overflow-menu .layer-overflow[data-parent-id='" + id + "']"
  );
  if (elem.hasClass("active")) {
    ...
    elem
      .addClass("active")
      .css({ top: top + 48, left: sliderProperties().width - 348 });
...
$(".osel-overflow-menu .layer-overflow .label.interactive").click(function () {
  if ($(this).data("function") == "extent") {
    ...
    if (typeof zoomToLayerExtent === "function") zoomToLayerExtent(layer);
...
$(
  ...
  function () {
    ...
    if (elem[0].offsetWidth < elem[0].scrollWidth)
      $("<div/>", { class: "osel-tooltip", text: elem.text() })
...
$(".osel-control-container .action-button").click(function () {
  ...
  if (elem.parent().hasClass("center-control")) {
    $(".osel-control-container .basemap-control .action-button").removeClass(
      "selected"
    );
    $(".osel-map-crosshair").toggleClass("hidden");
  } else if (elem.parent().hasClass("basemap-control")) {
    $(".osel-control-container .center-control .action-button").removeClass(
      "selected"
    );
 ...
 */
$(
  ".osel-control-container .basemap-control .list-select .list-item [data-style='" +
    defaults.basemapStyle +
    "']"
).addClass("active");
