...
To achieve this, we can add some additional lines of code to the `'once'` event listener – before the zoom to extent method is called.
...
With this array of positions – we can then take the first and last values from the list to create a couple of point geometry features. These are added to the map through a new vector layer and source definition:
...
That's it! You can access the full version of the source code [here](https://labs.os.uk/public/os-api-resources/code-playground/quick-start/openlayers-adding-gpx).
