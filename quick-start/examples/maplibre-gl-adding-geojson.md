...
By default, MapLibre GL JS will add a thin outline to any added polygon layers. If we want to include a styled outline, we can to add another layer using exactly the same source – but this time rendering it as a line (`'type': 'line'`):
...
The downside to the above approach is that the GeoJSON data is only stored on the Web Worker side – meaning there is no way to actually reference the raw source.
...
The process is very similar to that which has been described above – we just need to use the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) to make the request for the **boundary.geojson** file – and treat the response as inline GeoJSON:
...
That's it! You can access the full version of the source code [here](https://labs.os.uk/public/os-api-resources/code-playground/quick-start/maplibre-gl-adding-geojson).
