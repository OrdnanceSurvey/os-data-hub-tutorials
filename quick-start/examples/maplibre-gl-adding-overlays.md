...
The point, line and polygon overlays can be added by defining a [GeoJSON](https://geojson.org/) FeatureCollection – and then adding it as a source layer to the map's style:
...
By default, MapLibre GL JS will add a thin outline to any added polygon layers. If we want to include a styled outline, we need to add another layer using exactly the same filtered source – but this time rendering it with the `'type'` property specified as *line*.
...
When a `'click'` event occurs on a feature in any of the queryable layers – we use the  [`queryRenderedFeatures`](https://maplibre.org/maplibre-gl-js/docs/API/classes/Map/#queryrenderedfeatures) method to show properties of clicked map elements.

This method takes a geometry (which in this case is a *5px* bounding box encompassing the clicked point) and the array of layer IDs – and returns an array of GeoJSON Feature objects representing visible features that satisfy the query parameters.
...
As an alternative to adding styled point features to the map (via GeoJSON) – we can use the following syntax to add a marker:
...
In a similar manner to the overlay features – we can attach a popup to the marker as follows:
...
That's it! You can access the full version of the source code [here](https://labs.os.uk/public/os-api-resources/code-playground/quick-start/maplibre-gl-adding-overlays).
